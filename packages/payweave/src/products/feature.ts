/**
 * `feature()` primitive (plans-and-features.md §1–§2, §9, PW-801).
 *
 ...
 * give the "did you mean" hint (§9). The symbol itself is never exported —
 ...
 *
 * Argument-shape validation is split by timing on purpose (§9, agent-playbook
...
/** Feature/plan id: lowercase alphanumeric, dashes/underscores, max 64 chars (§1, §3). */
export const PRODUCT_ID_REGEX = /^[a-z0-9][a-z0-9_-]{0,63}$/;

/** Shared with `plan.ts` — plans use the exact same id rule (§3). */
...
/** Module-private — carries the parsed {@link FeatureDef} on a feature callable (§9). */
const FEATURE_DEF: unique symbol = Symbol("payweave.products.featureDef");
/** Module-private — marks a genuine {@link FeatureInclusion} and its deferred-violation flag (§9). */
...
/** `feature()`'s accepted input — the bound for its `const` generic (§10). */
export type FeatureDefInput = z.input<typeof featureDefSchema>;
/** The two feature kinds (§1). */
export type FeatureType = FeatureDefInput["type"];

/** A feature's parsed definition, generic over its literal id/type (§10). */
export type FeatureDef<Id extends string = string, Type extends FeatureType = FeatureType> = {
  readonly id: Id;
  readonly type: Type;
};

/** The argument a metered feature's callable requires (§2). */
...
/**
 * One feature grant inside a plan's `includes` (§2, §9) — produced by CALLING
...
/** The `"boolean"` half of {@link FeatureInclusion} — narrowed so `FeatureFn`'s boolean overload doesn't widen back to the full union (§10). */
export type BooleanFeatureInclusion<Id extends string = string> = {
  readonly featureId: Id;
  readonly type: "boolean";
};

/** The `"metered"` half of {@link FeatureInclusion} — narrowed so `FeatureFn`'s metered overload doesn't widen back to the full union (§10). */
...
type InclusionMeta = {
  /** Set when a BOOLEAN feature's callable was invoked with an argument (§9) — `plan()` rejects it. */
...
/** The callable `feature()` returns — call it to produce a {@link FeatureInclusion} (§2). */
...
/**
 * Define a feature (§1). Validates `id` (regex) and `type` (enum) with Zod;
 * throws {@link PayweaveValidationError} on a bad definition. Returns a
 * callable — call it (§2) to produce a `FeatureInclusion` for a `plan()`'s
 * `includes` array.
 *
 * ```ts
 * const proModels = feature({ id: "pro_models", type: "boolean" });
...
export function feature<const Def extends FeatureDefInput>(
  ...
  const callable = (...args: unknown[]): FeatureInclusion => {
    if (parsed.type === "boolean") {
      // §9 deviation: never throw here — record the violation, `plan()` rejects it.
