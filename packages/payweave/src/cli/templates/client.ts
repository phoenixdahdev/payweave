/**
 * Optional frontend client file renderer (docs/v1/cli.md §1's artifact list:
 * "An optional client file for frontend use", PW-1005).
 *
 ...
 * never the SDK directly. Spec-silent decision: always scaffolded (no extra
 * wizard prompt) — its role is "optional to use," not "optional to generate";
 * see `../init.ts`'s module doc comment for the full reasoning.
 */
import type { ScaffoldFile } from "./types";

export function renderClientFile(): ScaffoldFile {
  const contents = [
    "// lib/payweave-client.ts — optional helper for frontend code (cli.md §1).",
