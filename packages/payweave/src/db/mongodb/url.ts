/**
 * Input resolution for `mongodbAdapter(...)`.
 *
 ...
 * means classifying an input needs no driver import at all (adapters only
 * import their driver lazily, inside their own subpath).
 *
 ...
 * `mongodb+srv://` only. Never echoes `url` back in the
  ...
  if (typeof dbName !== "string" || dbName.length === 0) {
    throw new PayweaveConfigError(
      "payweave/db/mongodb: mongodbAdapter(...) requires a non-empty `dbName` — a MongoDB " +
        ...
        "requires `dbName` explicitly, for both { url } and { client }.",
    );
  }
}
